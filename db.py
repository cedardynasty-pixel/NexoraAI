"""
db.py — persistence layer for Nova Learn.
...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "nova_learn.db")

# Plan tiers, lowest to highest access. Keep this order in sync with the
# pricing cards in views/home.py.
PLAN_ORDER = ["Normal Pack", "Inspire Pack", "Pro Pack"]
...
class _DictCursor:
    """Wraps a libsql cursor so fetchone/fetchall return _Row objects (dict + index access)."""

    def __init__(self, raw_cursor):
        self._cursor = raw_cursor

    def _cols(self):
        return [c[0] for c in self._cursor.description] if self._cursor.description else []
...
class _LibsqlConn:
    """Wraps a raw libsql connection so conn.execute(...) returns dict-and-index-accessible rows."""
...
def get_conn():
    ...
    if turso_url and turso_token:
        ...
        conn = _LibsqlConn(libsql.connect(database=turso_url, auth_token=turso_token))
    else:
...
def init_db():
    with get_conn() as conn:
        ...
        if "plan" not in existing_cols:
            conn.execute("ALTER TABLE users ADD COLUMN plan TEXT NOT NULL DEFAULT 'Normal Pack'")
        conn.execute(
            """
        ...
        existing_book_cols = [row["name"] for row in conn.execute("PRAGMA table_info(books)")]
        if "required_plan" not in existing_book_cols:
            conn.execute("ALTER TABLE books ADD COLUMN required_plan TEXT NOT NULL DEFAULT 'Normal Pack'")
        conn.execute(
            """
...
def create_user(
    username: str, email: str, password: str, grade: str, is_admin: bool = False, plan: str = "Normal Pack"
) -> tuple[bool, str]:
    ...
    with get_conn() as conn:
        conn.execute(
            "INSERT INTO users (username, email, grade, salt, password_hash, created_at, is_admin, plan) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (username, email, grade, salt, pwd_hash, datetime.utcnow().isoformat(), int(is_admin), plan),
        )
...
def get_user(username: str) -> sqlite3.Row | None:
    with get_conn() as conn:
        return conn.execute(
            "SELECT id, username, email, grade, created_at, is_admin, plan FROM users WHERE username = ?",
            (username,),
        ).fetchone()


def set_user_plan(username: str, plan: str):
    if plan not in PLAN_ORDER:
        return
    with get_conn() as conn:
...
def get_all_users():
    with get_conn() as conn:
        return conn.execute(
            "SELECT id, username, email, grade, created_at, is_admin, plan FROM users ORDER BY created_at DESC"
        ).fetchall()
...
def create_book(title: str, created_by: str, required_plan: str = "Normal Pack") -> int:
    if required_plan not in PLAN_ORDER:
        required_plan = "Normal Pack"
    with get_conn() as conn:
        cur = conn.execute(
            "INSERT INTO books (title, created_by, created_at, required_plan) VALUES (?, ?, ?, ?)",
            (title, created_by, datetime.utcnow().isoformat(), required_plan),
        )
        return cur.lastrowid


def set_book_plan(book_id: int, required_plan: str):
    if required_plan not in PLAN_ORDER:
        return
    with get_conn() as conn:
        conn.execute("UPDATE books SET required_plan = ? WHERE id = ?", (required_plan, book_id))
...
def get_all_books():
    with get_conn() as conn:
        return conn.execute(
            """
            SELECT b.id, b.title, b.created_by, b.created_at, b.required_plan,
...
def get_books_for_student(student_plan: str):
    """Only books the student's plan tier can access (their plan rank >= book's required rank)."""
    student_rank = plan_rank(student_plan)
    return [b for b in get_all_books() if plan_rank(b["required_plan"]) <= student_rank]

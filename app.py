"""
app.py — Nova Learn Streamlit app entry point.
...
st.set_page_config(page_title="Nova Learn | Learn Without Limits", page_icon="🎓", layout="wide")
...
db.seed_default_admin(
    ...
    email=st.secrets.get("ADMIN_EMAIL", "admin@novalearn.local"),
    password=st.secrets.get("ADMIN_PASSWORD", "changeme123"),
)

# ---- Sidebar branding ----
with st.sidebar:
    st.markdown("## 🎓 NOVA LEARN")
    if is_logged_in():
